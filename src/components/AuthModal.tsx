      ...
      <div className="relative w-full max-w-md rounded-[28px] bg-white p-8 shadow-[0_35px_80px_-35px_rgba(21,128,61,0.55)] border border-primary/10">
        ...
        <div className="mb-6 text-center">
          <h2 className="text-2xl font-bold text-gray-900">
            {mode === "login" ? "Welcome back" : mode === "reset" ? "Reset your password" : "Create account"}
          </h2>
          <p className="mt-1 text-sm text-gray-500">
            {mode === "login"
              ? "Sign in to your NaijaStays account"
              : mode === "reset" ? "We will email you a secure reset link"
        ...
        {mode !== "reset" && <div className="relative mb-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? "Please wait…" : mode === "login" ? "Sign In" : mode === "reset" ? "Email reset link" : "Create Account"}
          </button>
        </form>

        <p className="mt-4 text-center text-sm text-gray-500">
          {mode === "reset" ? (
            <button onClick={() => setMode("login")} className="inline-flex items-center gap-1 font-medium text-primary hover:underline"><ArrowLeft size={14} /> Back to sign in</button>
          ) : mode === "login" ? (
            <>
              ...
              <button onClick={() => setMode("reset")} className="ml-3 font-medium text-primary hover:underline">Forgot password?</button>
            </>
          ) : (
            <>
